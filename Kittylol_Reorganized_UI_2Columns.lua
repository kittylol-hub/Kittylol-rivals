...
local Library = { 
    ...
    local Themes = {
        ["Preset"] = {
            ["Border"] = Color3.fromRGB(35, 8, 28),
            ["Outline"] = Color3.fromRGB(105, 25, 75),
            ["Background"] = Color3.fromRGB(20, 5, 17),
            ["Inline"] = Color3.fromRGB(30, 8, 25),
            ["Accent"] = Color3.fromRGB(255, 105, 180),
            ["Text"] = Color3.fromRGB(255, 225, 242),
            ["Inactive Text"] = Color3.fromRGB(205, 145, 180),
            ["Element"] = Color3.fromRGB(55, 15, 43),
            ["Element 2"] = Color3.fromRGB(75, 20, 58),
            ["Hovered Element"] = Color3.fromRGB(95, 25, 72)
        }
    }
    ...
    do
        ...
        Library.Window = function(Self, Params)
            ...
            local Items = { } do 
                ...
                Items["Galaxy"] = Library:Create("Frame", {
                    Name = "Galaxy",
                ...
                Library:Create("UIGradient", {
                    Name = "GalaxyGradient",
                    Parent = Items["Galaxy"].Instance,
                    Rotation = 35,
                    Color = ColorSequence.new({
                        ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 4, 24)),
                        ColorSequenceKeypoint.new(0.25, Color3.fromRGB(75, 8, 55)),
                        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(25, 4, 28)),
                        ColorSequenceKeypoint.new(0.75, Color3.fromRGB(90, 10, 65)),
                        ColorSequenceKeypoint.new(1, Color3.fromRGB(20, 3, 22))
                    })
                })
                ...
                for i = 1, 34 do
                    local Star = Library:Create("TextLabel", {
                        Name = "Star",
                        Parent = Items["Galaxy"].Instance,
                        BackgroundTransparency = 1,
                        Text = (i % 7 == 0) and "✦" or "•",
                        Font = Enum.Font.GothamBold,
                        TextColor3 = (i % 5 == 0) and Color3.fromRGB(255, 190, 230) or Color3.fromRGB(255, 105, 180),
                        TextSize = StarRandom:NextInteger(5, 12),
                        TextTransparency = StarRandom:NextNumber(0.15, 0.75),
                        Position = UDim2.new(StarRandom:NextNumber(0.02, 0.96), 0, StarRandom:NextNumber(0.06, 0.96), 0),
                        Size = UDim2.fromOffset(18, 18),
                        ZIndex = 1
                    })
                    table.insert(GalaxyStars, Star)
                end

                task.spawn(function()
                    while Items["Galaxy"].Instance and Items["Galaxy"].Instance.Parent do
                        local Star = GalaxyStars[StarRandom:NextInteger(1, #GalaxyStars)]
                        if Star and Star.Instance then
                            local target = Star.Instance.TextTransparency > 0.5 and 0.05 or 0.85
                            Library:Thread(function()
                                Star:Tween({TextTransparency = target}, TweenInfo.new(0.65, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut))
                            end)
                        end
                        task.wait(0.08)
                    end
                ...
                local GalaxyGlow = Library:Create("Frame", {
                    Name = "GalaxyGlow",
                    Parent = Items["Galaxy"].Instance,
                    AnchorPoint = Vector2.new(0.5, 0.5),
                ...
                Library:Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = GalaxyGlow.Instance})
...
-- The old Kittylol-built UI has been completely removed.  This adapter maps the
-- existing feature API to the supplied Library.lua controls.
--==========================================================================
do
    local __Library = getgenv().Library
    if not __Library then
        error("Library.lua failed to load")
    end

    local function attachOption(flag, object)
        if not object then return object end

        local key = flag or object.Flag or object.Name
        KittylolUI.Options[key] = object

        -- The old feature code uses Fluent-style OnChanged and SetValue.
        -- Library.lua exposes Callback and Set, so bridge those APIs here.
        local originalCallback = object.Callback
        local changedCallbacks = {}

        object.Callback = function(value, ...)
        ...
        if type(object.Set) == "function" and type(object.SetValue) ~= "function" then
    ...
    -- The new Library uses Window() and Page(), while the feature code uses
    -- CreateWindow() and AddTab().
    function __Library.CreateWindow(Self, config)
        config = config or {}

        if config.MinimizeKey then
            Self.MenuKeybind = tostring(config.MinimizeKey)
        end

        local window = Self:Window({
            Name = config.Title or config.Name or "Kittylol Hub"
        })

        function window:AddTab(tabConfig)
            tabConfig = tabConfig or {}
            return self:Page({
                Name = tabConfig.Title or tabConfig.Name or "Tab"
            })
        end

        function window:Dialog(data)
            -- Library.lua has notifications rather than the old custom modal.
            -- Keep the feature flow intact without creating another UI system.
            data = data or {}
            Self:Notification(
                tostring(data.Title or "Confirmation"),
                3,
                Self.Theme.Accent
            )
            ...
            for _, button in ipairs(buttons) do
                if button and (button.Title == "Yes" or button.Title == "OK" or button.Title == "Confirm") then
                    task.defer(function()
        ...
        window.Instance = window.Items and window.Items["Outline"] and window.Items["Outline"].Instance
        ...
        function window:SelectTab(index)
            local page = self.Pages and self.Pages[index]
            if page and type(page.Turn) == "function" then
                page:Turn()
            end
        end

        KittylolUI.Window = window
        KittylolUI.ScreenGui = Self.Holder and Self.Holder.Instance
        return window
    end

    -- New Library controls -> old feature API names.
    function __Library.AddSection(Self, name)
        return Self:Section({Name = tostring(name or "Section")})
    end

    function __Library.AddToggle(Self, flag, config)
        config = config or {}
        local object = Self:Toggle({
            Name = config.Title or config.Name or flag,
            Flag = flag,
            Default = config.Default == nil and false or config.Default,
            Callback = config.Callback
        })
        return attachOption(flag, object)
    end

    function __Library.AddSlider(Self, flag, config)
        config = config or {}
        local object = Self:Slider({
            Name = config.Title or config.Name or flag,
            Flag = flag,
            Default = config.Default == nil and 0 or config.Default,
            Min = config.Min == nil and 0 or config.Min,
            Max = config.Max == nil and 100 or config.Max,
            Decimals = config.Decimals == nil and (config.Rounding or 0) or config.Decimals,
    ...
    function __Library.AddDropdown(Self, flag, config)
        config = config or {}
        local object = Self:Dropdown({
            Name = config.Title or config.Name or flag,
            Flag = flag,
            Items = config.Values or config.Items or {},
    ...
    function __Library.AddKeybind(Self, flag, config)
        config = config or {}
        local label = Self:Label({
            Name = config.Title or config.Name or flag
        })
        local object = label:Keybind({
            Name = config.Title or config.Name or flag,
            Flag = flag,
            Default = config.Default,
    ...
    function __Library.AddColorpicker(Self, flag, config)
        config = config or {}
        local label = Self:Label({
            Name = config.Title or config.Name or flag
        })
        local object = label:Colorpicker({
            Name = config.Title or config.Name or flag,
            Flag = flag,
    ...
    function __Library.AddButton(Self, config)
        config = config or {}
        return Self:Button({
            Name = config.Title or config.Name or "Button",
            Callback = config.Callback
        })
    end

    function __Library.AddLabel(Self, value)
        if type(value) == "table" then
            return Self:Label(value)
        end
        return Self:Label({Name = tostring(value or "Label")})
    end

    function __Library.AddInput(Self, flag, config)
        config = config or {}
    ...
    -- Compatibility for the few existing references that still use the
    -- KittylolUI namespace.
    KittylolUI = {
        Options = {},
        Theme = __Library.Theme,
        Window = nil,
        ScreenGui = __Library.Holder and __Library.Holder.Instance
    }

    function KittylolUI:Notify(data)
        data = data or {}
        return __Library:Notification(
            tostring(data.Title or "Kittylol Hub"),
            tonumber(data.Duration) or 3,
            data.Color or __Library.Theme.Accent
        )
    end

    function KittylolUI:SetTheme(theme)
        theme = theme or {}
        local accent = theme.Accent or theme.Primary
        if typeof(accent) == "Color3" then
            __Library:ChangeTheme("Accent", accent)
        end
    end

    -- Config compatibility uses the new Library's GetConfig/LoadConfig
    -- implementation; no old UI/config implementation is retained.
    local ConfigCompat = {
        Folder = "KittylolRivals/config",
        Ignore = {}
    }
    ...
    function ConfigCompat:SetFolder(folder)
        self.Folder = tostring(folder or self.Folder)
        ensureFolder(self.Folder)
    end

    function ConfigCompat:SetIgnoreIndexes(indexes)
        self.Ignore = {}
    ...
    function ConfigCompat:List()
        local result = {}
        if type(listfiles) ~= "function" then return result end
        ensureFolder(self.Folder)
        local files = listfiles(self.Folder)
        for _, file in ipairs(files) do
            if file:sub(-5) == ".json" and not file:lower():match("[\\/]autoload%.json$") then
                local name = file:match("[^\\/]+$")
                if name then
                    table.insert(result, name:sub(1, -6))
                end
            end
        end
        table.sort(result)
    ...
    function ConfigCompat:Save(name)
        if type(writefile) ~= "function" then return false, "writefile is unavailable" end
        ensureFolder(self.Folder)
        local safe = tostring(name or ""):gsub("[^%w_%-%s]", "")
        if safe == "" then return false, "invalid config name" end
        local ok, err = pcall(function()
            writefile(self.Folder .. "/" .. safe .. ".json", __Library:GetConfig())
        end)
        return ok, ok and "saved" or tostring(err)
    end

    function ConfigCompat:Load(name)
        ...
        local ok, result = pcall(function()
            return __Library:LoadConfig(readfile(path))
        end)
        return ok and result ~= false, result
    end

    function ConfigCompat:Delete(name)
    ...
    function ConfigCompat:SetAutoload(name)
        if type(writefile) ~= "function" then return false, "writefile is unavailable" end
        local path = self.Folder .. "/" .. tostring(name) .. ".json"
        if type(isfile) == "function" and not isfile(path) then
            return false, "config does not exist"
        ...
        ensureFolder(self.Folder)
        local ok, err = pcall(function()
            writefile(self.Folder .. "/autoload.json", readfile(path))
        end)
        return ok, ok and "autoload set" or tostring(err)
    end

    function ConfigCompat:LoadAutoload()
        if type(readfile) ~= "function" or type(isfile) ~= "function" then return end
        local path = self.Folder .. "/autoload.json"
        if not isfile(path) then return end
        local content = readfile(path)
        if content and content ~= "" then
            pcall(function() __Library:LoadConfig(content) end)
        end
    end
    ...
    function KittylolUI:BuildConfigSection(tab)
        ...
        local names = ConfigCompat:List()
        local selected = names[1]
        local nameBox = section:AddInput("config_name", {
            Title = "Config Name",
            Default = "",
            Finished = true
        })

        local picker = section:AddDropdown("config_select", {
            Title = "Config",
            Values = names,
            Default = selected,
            Callback = function(value)
        ...
        section:AddButton({
            Title = "Save Config",
            Callback = function()
                local name = tostring(nameBox.Value or "")
                local ok, result = ConfigCompat:Save(name)
                self:Notify({
        ...
        section:AddButton({
            ...
            Callback = function()
                if not selected then
                    self:Notify({Title = "Config", Content = "Select a config first.", Duration = 3})
                    return
                end
                local ok, result = ConfigCompat:Load(selected)
                self:Notify({
        ...
        section:AddButton({
            ...
            Callback = function()
                ...
                local ok, result = ConfigCompat:Delete(selected)
                self:Notify({
    ...
    -- Keep the existing feature declarations below unchanged by exposing the
    -- new library through the same Fluent/Kittylol names they expect.
    Fluent = __Library

    local oldCreateWindow = __Library.CreateWindow
    -- Set the public options table only after CreateWindow is called.
    -- The wrapper above stores controls here through attachOption().
    local originalCreateWindow = oldCreateWindow
    __Library.CreateWindow = function(Self, config)
        local window = originalCreateWindow(Self, config)
        return window
    end

    -- Replace the initial empty options reference now that KittylolUI exists.
    Options = KittylolUI.Options

    Brand = {
        Accent = __Library.Theme.Accent,
        Text = __Library.Theme.Text,
        Sub = __Library.Theme["Inactive Text"]
    }

    Notify = function(data)
        return KittylolUI:Notify(data)
    end
...
-- Load the supplied new UI library.  If this merged file is used, Library.lua
-- is embedded above, so no external UI implementation is needed.
local __NewLibrary = getgenv().Library
local Fluent = __NewLibrary

local screenSize = Services.Camera and Services.Camera.ViewportSize or Vector2.new(1920, 1080)
...
local Tabs = {
    AimbotUtility = Window:AddTab({ Title = "Aimbot | Utility", Icon = "crosshair" }),
    WeaponsPlayer = Window:AddTab({ Title = "Weapons | Player", Icon = "package" }),
    ESPMisc = Window:AddTab({ Title = "ESP | Miscellaneous", Icon = "eye" }),
    SkinsSettings = Window:AddTab({ Title = "Skins | Settings", Icon = "brush" }),
    Discord = Window:AddTab({ Title = "Kittylol Discord", Icon = "message-circle" })
}

-- Aliases keep the existing feature code working while the visible UI is
-- reorganized into the requested two-column tabs.
Tabs.Aimbot = Tabs.AimbotUtility
Tabs.Utility = Tabs.AimbotUtility
Tabs.Weapons = Tabs.WeaponsPlayer
Tabs.Player = Tabs.WeaponsPlayer
Tabs.ESP = Tabs.ESPMisc
Tabs.Misc = Tabs.ESPMisc
Tabs.Skins = Tabs.SkinsSettings
Tabs.Settings = Tabs.SkinsSettings
Tabs.About = Tabs.Discord

-- Two-column placement: sections created before the marker below are
-- explicitly tagged as left/right so the new Library can lay them out.
local __KittylolLayout = {
    Aimbot = "Left",
    Utility = "Right",
    Weapons = "Left",
    Player = "Right",
...
local function __KittylolResolveSectionInstance(section)
    if not section then return nil end
    if typeof(section.Instance) == "Instance" then return section.Instance end
    if typeof(section.Frame) == "Instance" then return section.Frame end
    if typeof(section.Container) == "Instance" then return section.Container end
    if section.Items then
        for _, item in pairs(section.Items) do
            if type(item) == "table" and typeof(item.Instance) == "Instance" then
...
local function __KittylolSection(tab, name, side)
    local section = tab:AddSection(name)
    section.__KittylolSide = side
    return section
end

local function __KittylolMoveSection(section, side)
    if not section then return section end
    section.__KittylolSide = side

    local column = (side == "Left") and Window.KittylolLeftColumn or Window.KittylolRightColumn
    if not column then return section end

    local instance = __KittylolResolveSectionInstance(section)

    if typeof(instance) == "Instance" then
        pcall(function()
            instance.Parent = column
        end)
    end
...
--==========================================================================
-- KITTYLOL REAL TWO-COLUMN LAYOUT
--==========================================================================
do
    local __window = Window
    local __holder = __window and __window.Holder
    if __holder then
        local __content = __holder:FindFirstChild("Content") or __holder:FindFirstChild("Pages")
        if __content then
            local __columns = Instance.new("Frame")
            __columns.Name = "KittylolTwoColumns"
            __columns.BackgroundTransparency = 1
            __columns.BorderSizePixel = 0
            __columns.Size = UDim2.fromScale(1, 1)
            __columns.Position = UDim2.fromScale(0, 0)
            __columns.ZIndex = 1
            __columns.Parent = __content

            local __left = Instance.new("Frame")
            __left.Name = "LeftColumn"
            __left.BackgroundTransparency = 1
            __left.BorderSizePixel = 0
            __left.Size = UDim2.new(0.5, -5, 1, 0)
            __left.Position = UDim2.new(0, 0, 0, 0)
            __left.Parent = __columns

            local __right = Instance.new("Frame")
            __right.Name = "RightColumn"
            __right.BackgroundTransparency = 1
            __right.BorderSizePixel = 0
            __right.Size = UDim2.new(0.5, -5, 1, 0)
            __right.Position = UDim2.new(0.5, 5, 0, 0)
            __right.Parent = __columns

            local __leftLayout = Instance.new("UIListLayout")
            __leftLayout.Padding = UDim.new(0, 8)
            __leftLayout.SortOrder = Enum.SortOrder.LayoutOrder
            __leftLayout.Parent = __left

            local __rightLayout = Instance.new("UIListLayout")
            __rightLayout.Padding = UDim.new(0, 8)
            __rightLayout.SortOrder = Enum.SortOrder.LayoutOrder
            __rightLayout.Parent = __right

            -- Keep the two columns available for section placement.
            __window.KittylolLeftColumn = __left
            __window.KittylolRightColumn = __right
        end
    end
end

local __OriginalConfigTaskRemoved = true

local AimbotSection = Tabs.Aimbot:AddSection("Aimbot")
__KittylolMoveSection(AimbotSection, "Left")
...
__KittylolMoveSection(SilentAimSection, "Left")
...
__KittylolMoveSection(AimbotChecksSection, "Left")
...
__KittylolMoveSection(TriggerbotSection, "Left")
...
__KittylolMoveSection(OrbitSection, "Right")
...
__KittylolMoveSection(SkinsSection, "Left")
...
__KittylolMoveSection(InterfaceSection, "Right")
...
task.spawn(function()
    pcall(function()
        ...
        local __ConfigSection = KittylolUI:BuildConfigSection(Tabs.Settings)
        __KittylolMoveSection(__ConfigSection, "Right")
        KittylolUI.ConfigManager:LoadAutoload()
    end)
end)

local AboutSection = Tabs.About:AddSection("Kittylol Discord")
__KittylolMoveSection(AboutSection, "Right")

AboutSection:AddLabel("Discord: https://discord.gg/m4pYSx5WK")

AboutSection:AddButton({
    Title = " Join Discord",
    ActionText = "Copy",
    Callback = function()
        local link = "https://discord.gg/m4pYSx5WK"
        pcall(function()
...
__KittylolMoveSection(SocialsSection, "Right")
...
SocialsSection:AddButton({
    ...
    Callback = function()
        pcall(function()
            if setclipboard then
                setclipboard("https://discord.gg/m4pYSx5WK")
            elseif toclipboard then
                toclipboard("https://discord.gg/m4pYSx5WK")
            end
        end)
...
-- Final pass: move all requested sections after their controls are built.
task.defer(function()
    local __sections = {
        {AimbotSection, "Left"}, {SilentAimSection, "Left"},
        {AimbotChecksSection, "Left"}, {TriggerbotSection, "Left"},
        {OrbitSection, "Right"},
        {WeaponsSection, "Left"}, {PlayerSection, "Right"},
        {ESPSection, "Left"}, {MiscSection, "Right"},
        {SkinsSection, "Left"}, {InterfaceSection, "Right"},
        {SocialsSection, "Right"}, {AboutSection, "Right"},
    }

    for _, pair in ipairs(__sections) do
        if pair[1] then
            __KittylolMoveSection(pair[1], pair[2])
        end
    end
end)
