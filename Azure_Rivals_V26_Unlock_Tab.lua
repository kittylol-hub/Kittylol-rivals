...
function Library:CreateWindow(...)
    ...
    Library._SavePosFile = "AzureRivals_UI_Position.json"
...
local ThemeManager = {} do
        ...
        ThemeManager.Folder = "AzureRivalsHub"
...
LoadingScreenGui.Name = "AzureRivalsLoadingScreen"
...
LoadingTitle.Text = "Azure Rivals"
...
UnlockAllGroup = Tabs.Unlock:AddLeftGroupbox("Unlock All")
...
-- smooth feel.
RunService:BindToRenderStep("AzureRivalsAirMovement", Enum.RenderPriority.Character.Value, function()
    local char = lp.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    local root = char:FindFirstChild("HumanoidRootPart")
...
do
    ...
    AntiAim._conn1 = RunService.Heartbeat:Connect(function()
        ...
        if AntiAim.DesyncEnabled and lp.Character then
            local hrp = lp.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                pcall(function()
                    AntiAim._desyncCache = {
                        hrp.CFrame,
                        hrp.AssemblyLinearVelocity
                    }
                    local amp = AntiAim.DesyncAmp or 55
                    local t = tick()
                    local horizontalOsc = math.sin(t * 60) * amp
                    local verticalOsc = math.sin(t * 25) * (amp * 0.27)
                    local rotX = math.rad(math.random(-30, 30) / 10)
                    local rotY = math.rad(math.random(-30, 30) / 10)
                    local rotZ = math.rad(math.random(-30, 30) / 10)
                    local spoofed = hrp.CFrame
                        * CFrame.new(horizontalOsc, verticalOsc, 0)
                        * CFrame.Angles(rotX, rotY, rotZ)
                    hrp.CFrame = spoofed
                    hrp.AssemblyLinearVelocity = AntiAim._desyncCache[2]
                        + Vector3.new(math.cos(t * 8) * 6000, math.cos(t * 8) * 6000, 0)
                    RunService.RenderStepped:Wait()
                    if AntiAim._desyncCache and AntiAim._desyncCache[1] then
                        hrp.CFrame = AntiAim._desyncCache[1]
                        hrp.AssemblyLinearVelocity = AntiAim._desyncCache[2]
                    end
                end)
            end
            ...
            pcall(function()
                ...
                if char then
                    ...
                    if AntiAim._desyncCache and AntiAim._desyncCache[1] then
                        ...
                        if hrp then
                            hrp.CFrame = AntiAim._desyncCache[1]
                            if AntiAim._desyncCache[2] then
                                hrp.AssemblyLinearVelocity = AntiAim._desyncCache[2]
                            end
                        end
                        AntiAim._desyncCache = nil
                    end
                end
            end)
            if ForceThirdPerson and ForceThirdPerson.Enabled then
                pcall(function()
                    if ForceThirdPerson._origCameraMode then
                        lp.CameraMode = ForceThirdPerson._origCameraMode
                    end
                    if ForceThirdPerson._origMaxZoom then
                        lp.CameraMaxZoomDistance = ForceThirdPerson._origMaxZoom
                    end
                    if ForceThirdPerson._origMinZoom then
                        lp.CameraMinZoomDistance = ForceThirdPerson._origMinZoom
                    end
                end)
            end
        end,
...
RunService:BindToRenderStep("AzureRivalsAutoWalk", Enum.RenderPriority.Character.Value, function()
...
RunService.RenderStepped:Connect(function()
    ...
    if _fps.label then
        if PlayerSettings.ShowFPS then
            ...
            _fps.label.Text = "Azure Rivals | FPS: " .. _fps.current
        else
            _fps.label.Visible = false
        end
    end
...
do
    ...
    local function applyWeather()
        local existing = workspace:FindFirstChild("AzureRivalsWeatherPart")
        if existing then existing:Destroy() end
        if VisualSettings.WeatherType == "None" then return end
        local part = Instance.new("Part")
        part.Name = "AzureRivalsWeatherPart"
        part.Anchored = true
...
AntiAimGroup:AddToggle("AntiAimDesync", {
        Text = "Desync Mode (Azure)",
        Default = false,
})
Toggles.AntiAimDesync:OnChanged(function()
        AntiAim.DesyncEnabled = Toggles.AntiAimDesync.Value
        if AntiAim.DesyncEnabled then
                if getgenv().ZX_Notify then
                        getgenv().ZX_Notify("Anti-Aim", "Desync Mode enabled.", "success")
                end
        else
                if AntiAim._desyncCache and AntiAim._desyncCache[1] and lp.Character then
                        pcall(function()
                                local hrp = lp.Character:FindFirstChild("HumanoidRootPart")
                                if hrp and AntiAim._desyncCache[2] then
                                        hrp.CFrame = AntiAim._desyncCache[1]
                                        hrp.AssemblyLinearVelocity = AntiAim._desyncCache[2]
                                end
                        end)
                ...
                AntiAim._desyncCache = nil
                if getgenv().ZX_Notify then
                        getgenv().ZX_Notify("Anti-Aim", "Desync Mode disabled.", "warning")
                end
        end
end)

AntiAimGroup:AddSlider("AntiAimDesyncAmp", {
        Text = "Desync Amplitude",
        Default = 12,
        Min = 1,
        Max = 20,
        Rounding = 0,
...
Options.AntiAimDesyncAmp:OnChanged(function()
        AntiAim.DesyncAmp = Options.AntiAimDesyncAmp.Value
end)
...
local ForceThirdPersonGroup = Tabs.Player:AddRightGroupbox("Force Third Person")
ForceThirdPersonGroup:AddToggle("ForceThirdPerson", {
        Text = "Enable Force 3rd Person",
        Default = false,
})
Toggles.ForceThirdPerson:OnChanged(function(state)
        ForceThirdPerson.Enabled = state
        if state then
                ForceThirdPerson._origCameraMode = lp.CameraMode
                ForceThirdPerson._origMaxZoom = lp.CameraMaxZoomDistance
                ForceThirdPerson._origMinZoom = lp.CameraMinZoomDistance
                lp.CameraMode = Enum.CameraMode.Classic
                lp.CameraMaxZoomDistance = ForceThirdPerson.MaxZoom
                lp.CameraMinZoomDistance = ForceThirdPerson.MinZoom
                if getgenv().ZX_Notify then
                        getgenv().ZX_Notify("Force 3rd Person", "Enabled.", "success")
                end
        else
                if ForceThirdPerson._origCameraMode then
                        lp.CameraMode = ForceThirdPerson._origCameraMode
                ...
                if getgenv().ZX_Notify then
                        getgenv().ZX_Notify("Force 3rd Person", "Disabled.", "warning")
                end
        end
end)

ForceThirdPersonGroup:AddSlider("FTPMaxZoom", {
...
Options.FTPMaxZoom:OnChanged(function()
        ForceThirdPerson.MaxZoom = Options.FTPMaxZoom.Value
        if ForceThirdPerson.Enabled then
                lp.CameraMaxZoomDistance = ForceThirdPerson.MaxZoom
        end
end)

ForceThirdPersonGroup:AddSlider("FTPMinZoom", {
...
Options.FTPMinZoom:OnChanged(function()
        ForceThirdPerson.MinZoom = Options.FTPMinZoom.Value
        if ForceThirdPerson.Enabled then
                lp.CameraMinZoomDistance = ForceThirdPerson.MinZoom
        end
end)
        ...
        while true do
                ...
                if ForceThirdPerson.Enabled then
                        pcall(function()
                                ...
                                if lp.CameraMaxZoomDistance < ForceThirdPerson.MaxZoom then
                                        lp.CameraMaxZoomDistance = ForceThirdPerson.MaxZoom
                                end
                                if lp.CameraMinZoomDistance > ForceThirdPerson.MinZoom then
                                        lp.CameraMinZoomDistance = ForceThirdPerson.MinZoom
                                end
                        end)
                end
...
UnlockAllGroup:AddToggle("EnableUnlockAllSwish", {
        ...
        Default = false,
        Tooltip = "Unlocks cosmetic items locally. Equip them in the game's loadout menu.",
})
Toggles.EnableUnlockAllSwish:OnChanged(function()
        if Toggles.EnableUnlockAllSwish.Value then
                task.spawn(function()
                        ...
                        if getgenv().ZX_Notify then
                                getgenv().ZX_Notify("Unlock All", "All cosmetics unlocked! Equip them in your loadout.", "success")
                        end
                end)
        end
end)
...
MenuGroup:AddButton({
        ...
        Func = function()
                pcall(function()
                        if isfile and isfile("AzureRivals_UI_Position.json") then
                                delfile("AzureRivals_UI_Position.json")
                        end
                end)
                Library:Notify({
                        Title = "Window",
...
-- ════════════════════════════════════════════════════════════════════
-- SILENT AIM — op src (adapted for Azure Rivals)
...
getgenv()._ZX_NameSpoof = (function()
    local NS = {
        Enabled = false,
        SpoofedName = "Azure Rivals",
        SpoofedDisplay = "Azure Rivals",
    ...
    local function setName(newName)
        NS.SpoofedName = newName or "Azure Rivals"
        NS.SpoofedDisplay = newName or "Azure Rivals"
...
ThemeManager:SetFolder("AzureRivalsHub")
SaveManager:SetFolder("AzureRivalsHub/game-config")
