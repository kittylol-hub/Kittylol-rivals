...
do
    ...
    function __Library.AddSection(Self, name, side)
        return Self:Section({
            Name = tostring(name or "Section"),
            Side = (side == "Right" or side == 2) and 2 or 1
        })
    end
...
local function __KittylolResolveSectionInstance(section)
    ...
    if section.Items and section.Items["Section"] then
        local item = section.Items["Section"]
...
local function __KittylolSection(tab, name, side)
    local sideIndex = (side == "Right") and 2 or 1
...
local function __KittylolMoveSection(section, side)
    ...
    local target = page and page.ColumnsData and page.ColumnsData[sideIndex]

    if instance and target and target.Instance then
        pcall(function()
            instance.Parent = target.Instance
        end)
    end
